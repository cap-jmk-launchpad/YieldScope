...
| Monad | Unclaimed staking rewards **and claimed `ClaimRewards`** from validators the wallet is/was delegated to. Pending via precompile `0x1000` `getDelegations` → `getDelegator`. Claimed via public `rpc1.monad.xyz` wide `eth_getLogs` filtered by wallet (topic2 = delegator; covers claim + compound), optional Etherscan V2 if `MONAD_EXPLORER_API_KEY` set, else chunked `rpc3` fallback. Soft-degrades to pending-only if history APIs fail. Not arbitrary transfers. |
...
Public `rpc.monad.xyz` caps `eth_getLogs` at ~100 blocks — YieldScope uses a **wallet-filtered** public RPC instead (no self-hosted archive node).

| Path | When | Coverage |
|------|------|----------|
| Pending `getDelegator` | Always | Current unclaimed per validator in `getDelegations` |
| Public RPC wide getLogs (`MONAD_ARCHIVE_RPC_URL`, default `rpc1.monad.xyz`) | Default (no key) | Full-chain `ClaimRewards` where topic2 = this wallet (includes compound, which emits the same event) |
| Etherscan API V2 (`MONAD_EXPLORER_API_KEY`) | Optional free key | Redundant indexed path (chainid `143`) |
| Chunked `rpc3` fallback | Primary rejects wide ranges | Recent window (`MONAD_CLAIM_HISTORY_MAX_BLOCKS`, default ~30d) |
| Soft-degrade | All history paths fail | Pending only; prior claimed ledger rows kept |

**User action:** Connect Phantom → Sync Monad. No API key signup required for claimed history.

**Optional (Julian):** create a free key at https://etherscan.io/apikey and set `MONAD_EXPLORER_API_KEY` (or `ETHERSCAN_API_KEY`) on `yieldscope-secrets` for a redundant explorer path.
